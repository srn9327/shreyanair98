...
def ping(host, timeout=1):
    ...
    delays = array('d', [0.0000, 0.0000, 0.0000, 0.0000])
    ...
    vars = vars.append({'min': str(round(packet_min, 4)), 'avg': str(round(packet_avg, 4)),
                        'max': str(round(packet_max, 4)), 'stddev': str(round(stdev_var, 4))},
                       ignore_index=True)
    print (vars)
    return vars
